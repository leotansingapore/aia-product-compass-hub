/**
 * Push Centurion PA lessons to Supabase as training_videos.
 *
 * Usage: node scripts/push-cpa-lessons-to-supabase.mjs
 */
...
const PRODUCT_ID = "centurion-pa";
const REST = `${SUPABASE_URL}/rest/v1/products?id=eq.${PRODUCT_ID}`;
const PROPS_DIR = resolve(__dirname, "../../my-video/.tmp/cpa-props");
const GH_RELEASE_BASE =
  "https://github.com/leotansingapore/aia-product-compass-hub/releases/download/cpa-videos";

// -- Lesson definitions (order matters) --
const LESSON_IDS = [
  "cpa-m1-l1", "cpa-m1-l2", "cpa-m1-l3", "cpa-m1-l4", "cpa-m1-l5", "cpa-m1-l6",
  "cpa-m2-l1", "cpa-m2-l2", "cpa-m2-l3", "cpa-m2-l4",
];
...
// Remove any previous cpa- entries before merging
const filtered = existing.filter((v) => !v.id.startsWith("cpa-"));
...
console.log(`  CPA entries:`);
for (const v of vids.filter((v) => v.id.startsWith("cpa-"))) {
