/**
 * Push Elite Secure Income lessons to Supabase as training_videos.
 *
 * Usage: node scripts/push-esi-lessons-to-supabase.mjs
 *
 * Reads lesson JSON + narration files from my-video/.tmp/esi-props/,
...
const PRODUCT_ID = "elite-secure-income";
const REST = `${SUPABASE_URL}/rest/v1/products?id=eq.${PRODUCT_ID}`;
const PROPS_DIR = resolve(__dirname, "../../my-video/.tmp/esi-props");

// -- Lesson definitions (order matters) --
const LESSON_IDS = [
  "esi-m1-l1", "esi-m1-l2", "esi-m1-l3", "esi-m1-l4",
  "esi-m1-l5", "esi-m1-l6", "esi-m1-l7",
  "esi-m2-l1", "esi-m2-l2", "esi-m2-l3", "esi-m2-l4", "esi-m2-l5",
];
...
// Remove any previous ESI entries (ids starting with "esi-")
const filtered = existing.filter((v) => !v.id.startsWith("esi-"));
...
for (const v of vids.filter((v) => v.id.startsWith("esi-"))) {
