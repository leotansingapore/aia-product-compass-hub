...
> **Where these products map in the live sales script:** the **[Canned Sales Track](/learning-track/first-60-days/reference/cst-risks-script-full)** introduces them as the "3 basic insurances" (Life, Hospitalisation, Personal Accident) on Page 1. The CST gives the client-facing language; this day gives you the actual AIA product names that fulfil each role.
...
|---|---|---|
| **Premature Death / TPD** | 10x gross annual income + major liabilities | $1M + $400K mortgage = **$1.4M** |
| **Critical Illness** | 3-5x annual income + **$100K buffer** | **$300K-$500K** |
| **Accident** | 5x annual income | **$500K** |
...
The $100K buffer on CI specifically funds **cancer drug treatments not covered by the hospital plan** (one vial can cost ~$10K per dose). The 3-5x income covers the recovery period when the client can't work - so the total benchmark is "$100K + 3-5x income" or equivalently "$200-400K or 5x income," which is the same coverage in two different framings.
...
Base sum x multiplier = total coverage **until the drop-off age (65 or 75)**. After drop-off, sum reverts to the base, but client is covered for life (premium fully paid by then).

| Multiplier | Base sum (for $500K total) | Drop-off options |
|---|---|---|
| **5x** | $100K x 5 | 65 or 75 |
| **3x** | ~$166K x 3 | 65 or 75 |
| **2x** | $250K x 2 | 65 or 75 |
...
### The closing technique that converts (3x -> 2x upgrade)
...
Always present 3x first. Then run the 2x comparison out loud:
...
4. ...up to 5 different conditions -> max payout **5 x $100K = $500K**
...
- Total premiums paid: ~$252K -> **3.25x return**
...
|---|---|---|---|
| **GPP** (2x or 3x multiplier) | $300-500K death + CI rider | Foundation - whole life with cash value | $7,000-$9,000 |
...
MOH maintains a **Cancer Drug List (CDL)** with approved drugs and a monthly claim limit per drug. MediShield Life pays only **1x the CDL limit**. AIA layers on top:

| Layer | CDL multiplier | Monthly cap |
|---|---|---|
| MediShield Life (base) | **1x** | e.g. $2K (varies by drug) |
| HealthShield Max A (base) | + **5x** | $10K |
| Cancer Care Booster rider | + **16x** | $32K |
| **Full package total** | **21x** | **~$42K/month** |

For **non-CDL drugs** (newer drugs not yet on the approved list): up to **$200K/year** with 10% co-insurance under the Booster rider.

For **cancer drug services** (scans, tests): Base 5x + Booster 10x = **15x total**.
...
The mechanism: use multiplier comparisons (3x vs 2x -> "$1,600 more for $275K more cash value") to make the upgrade feel obvious. They stretch when the value is undeniable.
...
**Premium profile:** 5-10x the cost of term for similar coverage, but coverage is permanent and builds cash value.
...
- **Major CI cover:** 5x annual income.
- **Early CI cover:** 2x annual income.
- **Death cover:** 10x annual income.
...
|---|---|---|
| Income replacement at death | Term or Whole Life | 10x annual income |
| Critical Illness | CI rider or standalone | 5x annual income (Major) + 2x (Early) |
...
2. **Why is Disability Income often overlooked?**
   - A) It's expensive
   - B) Clients assume CI covers this - but CI is a lump sum, not continuing income (correct)
...
3. **The biggest risk of relying on company hospital coverage is:**
   ...
   - B) Coverage ends with employment, and reapplying at older age may mean higher premiums or exclusions (correct)
...
5. **A freelance graphic designer with no employer sick leave asks what insurance is most often overlooked for people like her. What do you recommend addressing?**
   ...
   - B) Disability Income - replaces monthly income if she cannot work, which has no employer fallback (correct)
...
6. **Using the "layered protection" model, which layer should typically be established first for a 32-year-old breadwinner with two young children?**
   ...
   - C) Layer 1 - Term insurance for maximum coverage during the highest-need years (correct)
