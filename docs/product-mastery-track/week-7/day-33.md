---
week: 7
day: 33
title: "Platinum Wealth Venture — The Math + The Q&A — Healthcare Cost Angle, Color-Coded Review, Top 5 Questions"
primary_source: platinum-wealth-venture-product-summary
primary_slides: "n/a"
duration_minutes: 60
tags: [product-mastery-track, week-7, platinum-wealth-venture]
---
# Day 3 — The Math + The Q&A: Healthcare Cost Angle, Color-Coded Review, Top 5 Questions

> **Today's frame:** Move from problem-painting to problem-pricing. The healthcare angle works because pre-retirees already feel the pain — naming it as $412K converts vague worry into a sharp number. The asset-vs-expense reframe gives the client a way out. The color-coded policy review transforms FC from product-pitcher to portfolio-doctor. Then memorise the five questions every prospect asks.

This is a merged module — Part 1 covers healthcare angle + policy review numbers. Part 2 covers the top 5 questions. Estimated read + drill: ~60 minutes.

---

# Part 1 — The Numbers: Healthcare Angle + Color-Coded Policy Review

> **The one idea for Part 1:** Loss aversion (~2x stronger than gain motivation) is the conversion lever for pre-retirees. The healthcare angle ($412K lifetime premium) is loss made specific. The color-coded policy review (Green = Keep, Orange = Optimize, Red = Restructure/Surrender) makes the recommendation visual. Both convert PWV from a product to a portfolio decision.

By the end of Part 1 you should be able to recite the healthcare premium escalation numbers, deploy the asset-vs-expense reframe, classify any existing policy as Green/Orange/Red, and quote the PWV fee architecture cold.

---

## The Healthcare Cost Angle (the loss aversion hook)

This is the single most powerful emotional driver for pre-retirees. Every client already feels the pain of rising hospital premiums — the FC names it as a number.

The opener:

> *"Mr Tan, are you aware that based on your HealthShield Gold Max A-Plan, from age 55 to age 85, you will spend a total of $412,000 just to pay for this hospitalization plan premium?"*

[Video 2, Video 23]

The pain reinforcement:

> *"If you don't do anything today, you are still going to spend this $412,000 no matter what. Just for hospitalization plan."*

> *"And this is based on today's figures. The premium has been adjusted every 2-3 years over the past decade because of healthcare inflation."*

## The asset-vs-expense reframe (the conversion)

> *"Instead of paying this $400,000 to the insurer as an expense, what if we set this money aside today to build an asset for yourself? You'd generate enough income to pay for your hospitalization plan premium AND have excess for your lifestyle AND leave the capital to your family when you pass on."*

> *"Even at 6% dividend yield, we cover all the premiums. Even at 5%, we still cover all the premiums. But what our clients are actually getting is around 7%."*

The math:

```
$412,000 of HSGM premium over 30 years (age 55-85)
vs
$200K-$400K invested in PWV at 6% dividend
=
PWV income that COVERS the HSGM premium AND has excess for lifestyle AND preserves capital for family
```

## The "if you downgrade to public" pre-emption

If the prospect threatens to downgrade hospital cover:

> *"Once you downgrade to public, the cost drops — for yourself. But what about for your wife? Now we're looking at $600,000 for both of you."*

The number doubles. Most prospects rebound.

## Healthcare premium escalation numbers

| Item | Value |
|---|---|
| HSGM A lifetime premium (age 55-85) | **~$412,000** [Video 2, 23] |
| HSGM A current annual at age 55 | ~$565/year |
| HSGM A annual at age 90 | ~$3,006/year |
| Lifetime hospital premium range across plan types | **$91,000-$171,000 per person** [Video 23] |
| Premium escalation cadence | Every 2-3 years per AIA repricing cycle |

## The Color-Coded Policy Review (Green / Orange / Red)

After the healthcare angle, walk through the prospect's existing policies (pulled from SingPass at Phase 3). Label each:

### Green = KEEP
> *"Green means Keep — good value, adequate coverage, reasonable premiums."*

Examples:
- HSGM (regardless of plan tier — keep, possibly Optimize at the rider level)
- Recently bought CI plans with strong rider stack
- Term plans at favourable legacy premium rates

### Orange = OPTIMIZE
> *"Orange means Optimize — same plan, but we improve riders, switch funds, or change payment frequency."*

Examples:
- HSGM rider could be downgraded or upgraded
- Annual vs monthly payment (monthly costs ~2-3% more)
- Junior policy that could be upgraded to adult coverage

Premium savings examples [Video 19]:
| Optimisation | Annual savings |
|---|---|
| Rider downgrade A-plan to Value | ~S$1,400/year |
| Annual vs monthly payment | 2-3% |
| Junior to adult policy upgrade | 3x coverage for $30 more |

### Red = RESTRUCTURE / SURRENDER
> *"Red means Restructure or Surrender — poor returns, redundant coverage, or excessive cost."*

Examples (real client cases from Video 19):
- **NTUC whole life** — yielding 1.07% on long-held policy
- **GE Living Assurance** — $48K cash value vs $50K death after 29 years (protection essentially gone)
- **Pro Achiever ILP** — $2,800 growth over 7 years (~1.4% IRR)
- **Prudential** — cost of insurance escalation ($11/$1K at 55 → $67/$1K at 69)

The reframe script:

> *"Your NTUC whole life is yielding 1.07% — your bank fixed deposit gives you 2.5%. By restructuring this and your other two red policies, you'll save $7,000 per year. That's money that could be working for you instead."*

## The PWV fee architecture (must know cold)

Memorise these from PS Section 5:

| Charge | Value | Duration |
|---|---|---|
| **Top-up Premium Charge** | 3% of top-up | Per top-up [PS p.4, Section 5.1] |
| **Supplementary Charge** | 3.60% p.a. of Regular Premium Policy Value | Monthly, **first 10 policy years only** [PS p.4, Section 5.2] |
| **Premium Holiday Charge** | 60% (early years) tapering to 0% from year 8 | If regular premium not paid by due date [PS p.5, Section 5.4] |
| **Full Surrender Charge** | 70% Y1 / 50% Y5 / 30% Y7 / 0% Y11+ | [PS p.5, Section 5.5] |
| **Partial Withdrawal Charge factor** | Y1 = 2.333 / Y8 = 0.250 / Y11+ = 0 | [PS p.4, Section 5.6] |
| **Benefit Charge** | Per Sum-at-Risk per year | Age/gender-rated [PS p.14-15, Appendix A] |

Sample Benefit Charges (per S$1,000 SAR, annual):

| Age | Male | Female |
|---|---|---|
| 50 | $3.59 | $2.39 |
| 55 | $7.12 | $5.22 |
| 60 | $10.89 | $8.44 |

[PS p.14-15, Appendix A]

## The 4 PWV bonus types (offset against the fees)

| Bonus | Value | Trigger |
|---|---|---|
| **Welcome Bonus** | 16% / 26% / 35% over 5 years | Premium tier ($7.8K-$8.99K / $9K-$11.99K / $12K+) [PS p.2, Section 3.3] |
| **Investment Bonus** | 2.5% per year | Years 9, 10, 11, 12 [PS p.2, Section 3.4] |
| **Performance Bonus** | 0.30% p.a. of Regular Premium Policy Value | From year 9 onwards [PS p.2, Section 3.5] |

Note: brochure communicates Performance Bonus as **0.4% from year 8** [Brochure p.2]. Use PS canonical (0.30% from year 9) in writing.

## Partial withdrawal mechanics

| Rule | Detail |
|---|---|
| Minimum partial withdrawal | S$1,000 |
| Minimum policy value remaining after | S$10,000 |
| Withdrawal charge factor | Year 1 = 2.333 down to Year 11+ = 0 |

[PS p.7, Section 6.4]

## The numbers cheat sheet

| # | Item | Value |
|---|---|---|
| 1 | HSGM A lifetime premium | ~$412,000 |
| 2 | Top-up Premium Charge | 3% per top-up |
| 3 | Supplementary Charge | 3.60% p.a., first 10 years only |
| 4 | Welcome Bonus tier 3 ($12K+) | 35% cumulative over 5 years |
| 5 | Investment Bonus | 2.5% × 4 years (years 9-12) |
| 6 | Performance Bonus | 0.30% p.a. from year 9 |
| 7 | Full Surrender Y1 charge | 70% |
| 8 | Full Surrender Y11+ charge | 0% |
| 9 | Partial Withdrawal Y1 factor | 2.333 |
| 10 | Free-look | 14 days |
| 11 | Min regular premium | S$7,800/year (S$650/mo) |
| 12 | Min top-up | S$1,000 |
| 13 | Min policy value after partial withdrawal | S$10,000 |
| 14 | Conservative dividend projection | 6% (always project at this) |
| 15 | Historical fund yield (AIA Global Adventurous Income) | ~7% p.a. since inception |

---

## Drill (Part 1)

1. **Recite the healthcare premium escalation numbers** ($412K HSGM A lifetime, $565 → $3,006/year escalation).
2. **Practise the asset-vs-expense reframe** in your own voice.
3. **Memorise the Color-Coded Policy Review framework** (Green/Orange/Red with examples).
4. **Memorise the PWV fee architecture** — Top-up 3%, Supplementary 3.60%, Surrender 70/50/30/0%.
5. **Memorise the 4 bonus types** with triggers.

---

# Part 2 — Top 5 Client Questions: Crisp, Scripted Answers

> **The one idea for Part 2:** Pre-retirees ask the same five questions in 80% of meetings. Memorise scripted answers with specific numbers from the Product Summary — the FC who quotes "3.60% supplementary charge for the first 10 policy years" (not "around 3.5%") is perceived as a technician. That perception shift is what turns a hesitant client into a buyer.

By the end of Part 2 you should be able to deliver each answer in under 60 seconds with the page citation, distinguish curiosity from objection, and run all 5 answers in random order.

---

## Q1 — "What's the dividend rate, and is it guaranteed?"

**A:** We project at 6% to be conservative. The AIA Global Adventurous Income Fund has historically achieved around 7% per annum since inception.

> *"Dividends are not guaranteed — that's regulated and I have to say it — but I'll show you actual quarterly payouts from real clients so you can see what's been happening, not just projections. Here's what a client with $120K invested received last quarter: $2,100. That's real money, not a projection."*

[Sources: Brochure p.5, PS p.13]

The discipline:
- **Always say "not guaranteed"**
- Always project at 6%, never 7% or 8%
- Show real client payouts when possible

## Q2 — "What happens to my money if I die?"

**A:** Capital floor — your family receives the **higher** of two amounts:
- Total premiums you've paid in (less any withdrawals)
- Policy value at the time

> *"So if your account value is $300K when you pass, they get $300K. If the market drops and your account is at $150K, they still get back at least the $200K you put in. That capital floor is a contractual feature of the policy."*

[Source: PS p.1, Section 3.1]

This is the single most reassuring fact about PWV. Always emphasise.

## Q3 — "Can I take my money out early if I need it?"

**A:** Yes, with two layers:

| Layer | Mechanic |
|---|---|
| **Partial withdrawal** | From day 1, but charge factor decreases yearly (Year 1 = 2.333 → Year 8 = 0.250 → Year 11+ = 0) |
| **Full surrender** | After year 7, no surrender charges (70% Y1 → 50% Y5 → 30% Y7 → 0% Y11+) |

> *"After year 7 there are no surrender charges on full surrender either. So the plan is built to be liquid after year 7. If you need access in years 1-7, you have it — you just pay a charge. Most of my clients never trigger this."*

[Sources: PS p.5, Section 5.5; PS p.7, Section 6.4]

## Q4 — "How are the dividends paid out?"

**A:** Quarterly. Two options:

### Option A — Cash payout
- Paid into wallet via PayNow NRIC every quarter
- Use however you want

### Option B — Reinvestment
- Dividends buy more units inside your fund
- Grows your future dividend base

> *"You don't have to decide today; you can switch between the two at any time by sending a service request, and from the next quarter the new option takes effect."*

[Source: PS p.10, Section 8.4]

## Q5 — "What are the actual fees?"

**A:** Three to know:

### 1. Supplementary Charge — 3.60% p.a.
On Regular Premium Policy Value, deducted monthly, **first 10 policy years only**. **Zero from year 11 onwards.** [PS p.4]

### 2. Premium Charge — 3%
On any top-up premiums you make. [PS p.4]

### 3. Benefit Charge — age/gender-rated
Per S$1,000 of Sum-at-Risk per year. Sample at age 55 male: $7.12; female: $5.22. [PS p.14-15]

### Against these fees, you receive bonuses:

| Bonus | Offset |
|---|---|
| Welcome Bonus | Up to 35% over years 1-5 |
| Investment Bonus | 2.5% per year in years 9-12 |
| Performance Bonus | 0.30% per year from year 9 onwards |

> *"The bonuses largely offset the fees, and after year 10 the supplementary charge stops while the bonuses continue."*

## Bonus question — "Why not just keep my money in fixed deposit at 2.85%?"

**A:** Three reasons:

1. **Capital floor** — FD has FDIC up to limits but no insurance death benefit; PWV has the higher-of capital guarantee
2. **Yield** — 6% PWV vs 2.5% FD = $7,000 extra annual income on $200K capital. Over 20 years, ~$300K of lost compounding
3. **Reset risk** — FDs roll at the prevailing rate; SIBOR drops, your income drops. PWV dividend frequency is quarterly and the underlying fund yield has historically been more stable than FD rates.

[Source: Video 30]

## The handover-to-Day-4 transition

If a prospect asks Q1-Q5 — they're curious. Today's answers handle them.

But if they push:
- *"I need to think about it"*
- *"I want to discuss with my spouse"*
- *"The market is too volatile"*
- *"I prefer fixed deposits — they're guaranteed"*
- *"I'll do it next year"*

— those are objections. Day 4 has the conviction stories.

---

## Drill (Part 2)

1. **Recite each of the 5 answers** out loud. Time each. Target: 60 seconds.
2. **Memorise the page citation for each** (Q1: Brochure p.5 / Q2: PS p.1 Section 3.1 / Q3: PS p.5, p.7 / Q4: PS p.10 Section 8.4 / Q5: PS p.4, p.14-15).
3. **Practise the "not guaranteed" language** in 3 different framings.
4. **Memorise the 5 bonus offsets to the fees** (Welcome 35% / Investment 2.5% × 4 / Performance 0.30%).
5. **Pair up with a peer** — have them ask the questions in random order. Score on accuracy + page citation.

---

## What's coming next

Day 4: defence + expansion. Part 1 — the 5 hardest objections (think-about-it, discuss-with-spouse, market-volatile, prefer-FDs, next-year). Part 2 — the APA → PWV restructure sequence (5-step audit → quantify → compare → document → redeploy) for prospects with legacy ILPs.

---

## Quiz
1. **According to Day 3, why does the healthcare premium angle work as the most powerful emotional driver for pre-retirees?**
- A) It is unique to AIA and competitors cannot match it
- B) Loss aversion is approximately 2x stronger than gain motivation, and the angle names existing pain as a specific number ✓
- C) It triggers urgency from MAS-imposed deadlines
- D) It is the only angle that satisfies RNF compliance

**Why:** Day 3 frames the healthcare angle as loss made specific — leveraging loss aversion (~2x stronger than gain motivation) by naming $412K of premium leakage.

2. **What does the Color-Coded Policy Review classify policies into?**
- A) Buy / Hold / Sell
- B) Pass / Fail / Review
- C) Green = Keep, Orange = Optimize, Red = Restructure / Surrender ✓
- D) Tier 1 / Tier 2 / Tier 3

**Why:** Day 3 defines Green = Keep, Orange = Optimize (riders/funds/payment frequency), Red = Restructure or Surrender.

3. **According to Day 3, what is the asset-vs-expense reframe?**
- A) Treating PWV as a tax-deductible expense rather than an asset
- B) Reclassifying CPF Life as an expense
- C) Setting capital aside today to build an asset that generates income to cover the hospitalization premium AND has excess for lifestyle AND preserves capital for family ✓
- D) Counting hospital premiums as part of the retirement income

**Why:** Day 3's verbatim reframe converts paying premium-as-expense into deploying capital-as-asset that covers premiums plus excess plus capital preservation.

4. **In Q1 (dividend rate), what dividend yield do you project with the client and what is the historical fund yield?**
- A) Project at 7%; historical 8% on AIA Global Dynamic Income Fund
- B) Project at 6% conservative; historical ~7% on AIA Global Adventurous Income Fund since inception ✓
- C) Project at 5%; historical 6%
- D) Project at 8%; historical 9%

**Why:** Day 3 disciplines: always project at 6%, never 7% or 8%; AIA Global Adventurous Income Fund historically achieves ~7% p.a. since inception (Brochure p.5, PS p.13).

5. **What is the PWV Supplementary Charge and how long does it apply?**
- A) 1.45% p.a. for the entire policy term
- B) 3.60% p.a. of Regular Premium Policy Value, monthly, first 10 policy years only ✓
- C) 5% p.a. for the first 5 years
- D) 3% per top-up indefinitely

**Why:** PS p.4 Section 5.2 (cited in Day 3): Supplementary Charge is 3.60% p.a. of Regular Premium Policy Value, deducted monthly, first 10 policy years only.

6. **According to Day 3's fee architecture, what is the Full Surrender Charge in Year 1 and Year 11+?**
- A) Y1 = 50%, Y11+ = 5%
- B) Y1 = 70%, Y11+ = 0% ✓
- C) Y1 = 100%, Y11+ = 10%
- D) Y1 = 30%, Y11+ = 0%

**Why:** PS p.5 Section 5.5 (cited in Day 3): Full Surrender Charge is 70% in Y1, sliding down to 0% from Y11 onwards.

7. **What is the minimum partial withdrawal amount and the minimum policy value remaining after withdrawal?**
- A) Min S$500 withdrawal; min S$5,000 remaining
- B) Min S$1,000 withdrawal; min S$10,000 remaining ✓
- C) Min S$2,000 withdrawal; min S$20,000 remaining
- D) Min S$5,000 withdrawal; min S$50,000 remaining

**Why:** PS p.7 Section 6.4 (cited in Day 3): minimum partial withdrawal is S$1,000 and policy value remaining after withdrawal must be at least S$10,000.

8. **In Q2 (death benefit), what does the client's family receive?**
- A) The total premiums paid only
- B) The current policy value only
- C) The HIGHER of total premiums paid (less withdrawals) OR current policy value ✓
- D) A guaranteed S$200,000 sum assured

**Why:** PS p.1 Section 3.1 (cited in Q2): the death benefit is the higher of total regular premium paid plus top-ups less withdrawals, or the policy value — the "capital floor."

9. **A prospect says: "Once I retire I'll just downgrade to a public hospital plan, so the $412K doesn't apply." Per Day 3's pre-emption script, what is the correct response?**
- A) "That's fine — let's not discuss healthcare anymore"
- B) "Once you downgrade, the cost drops for yourself. But what about your wife? Now we're looking at $600,000 for both of you" ✓
- C) "You can't downgrade — it's not allowed under MAS rules"
- D) "Public hospitals are unsafe, you shouldn't downgrade"

**Why:** Day 3 supplies this verbatim pre-emption — doubling the number to ~$600K when the spouse is included usually rebounds the prospect.

10. **A prospect asks: "Why not just keep my money in fixed deposit at 2.85%?" Day 3 lists three reasons. Which set is correct?**
- A) Tax efficiency, government backing, FDIC limits
- B) Capital floor, yield differential (~$7K/year on $200K), and FD reset risk when SIBOR drops ✓
- C) Currency hedging, MAS regulation, banking secrecy
- D) Higher liquidity, lower risk, faster access

**Why:** Day 3's bonus question lists capital floor (death benefit), yield differential ($7K/year on $200K = ~$300K over 20 years), and FD reset risk as the three reasons.
