...
// Deterministic tiered scorer instead of cmdk's fuzzy default. The default's
...
// the query "business plan". Tiers here: exact phrase in the value (4), phrase
// in keywords (3.5), every token an exact substring (3), then a fuzzy tier
// (0–2) where each token may subsequence-match with typos/omissions — so
// "prospcting" still finds Prospecting, but never outranks a literal hit.
...
function fuzzyTokenScore(token: string, hay: string): number {
...
function scoreEntry(value: string, search: string, keywords?: string[]): number {
  ...
  for (const t of tokens) {
    let s = Math.max(fuzzyTokenScore(t, v), fuzzyTokenScore(t, kw));
