...
import { Plus, Trash2, GripVertical, FileText, Copy, Upload } from "lucide-react";
import {
  DndContext,
  closestCenter,
  PointerSensor,
...
export default function AdminPhaseControls({
  ...
  return (
    <>
      ...
      <div className="px-4 py-2 border-t flex items-center gap-1 flex-wrap">
        <InlineEditableText
          value={phase.title}
        ...
        <button
          type="button"
          onClick={() => handleCopyPhase(otherTrack)}
          ...
          {addingItem ? (
            <div className="flex items-center gap-2">
              <input
                ...
                autoFocus
              />
              <button onClick={handleAddItem} disabled={!newItemTitle.trim()} className="px-2 py-1 text-xs bg-primary text-primary-foreground rounded disabled:opacity-40">Add</button>
              <button onClick={() => { setAddingItem(false); setNewItemTitle(""); }} className="px-2 py-1 text-xs text-muted-foreground">Cancel</button>
            </div>
          ) : showTemplates ? (
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => setShowTemplates(true)} className="flex items-center gap-1 text-xs text-primary hover:underline">
                <FileText className="h-3 w-3" />Add from template
              </button>
              <span className="text-muted-foreground/40">·</span>
              <button onClick={() => setAddingItem(true)} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground">
                <Plus className="h-3 w-3" />Quick blank item
              </button>
              <span className="text-muted-foreground/40">·</span>
              <button onClick={() => setBulkImportOpen(true)} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground">
