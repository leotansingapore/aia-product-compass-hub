  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger asChild>
        <button className="flex w-full items-center justify-between rounded-lg border bg-card p-4 text-left transition-colors hover:bg-accent/50">
        ...
        <div className="mt-2 space-y-2 pl-2">
          {week.sessions.map((session) => (
            <TrackItemRow
              key={session.id}
              item={session}
              isCompleted={progressHook.isCompleted(session.id)}
              onToggle={() => progressHook.toggleItem(session.id)}
            />
          ))}
        </div>
      </CollapsibleContent>
