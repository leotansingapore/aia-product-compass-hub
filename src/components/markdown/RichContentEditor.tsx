...
function detectVideoUrl(url: string): { platform: string; embedUrl: string } | null {
  const patterns = [
    ...
    { regex: /vimeo\.com\/(?:manage\/videos\/|video\/)?(\d+)(?:\/([a-f0-9]+))?/, platform: 'vimeo', embed: (_id: string, match: RegExpMatchArray) => { const hash = match[2] ? `?h=${match[2]}` : ''; return `https://player.vimeo.com/video/${match[1]}${hash}`; } },
