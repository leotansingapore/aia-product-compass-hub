...
// being viewed is downloaded. The hub and navigation use DAY_SUMMARIES.
const rawLoaders = import.meta.glob<string>("/docs/first-14-days/week-*/day-*.md", {
  query: "?raw",
  import: "default",
});

// Week README loaders — same lazy pattern.
const weekReadmeLoaders = import.meta.glob<string>(
  "/docs/first-14-days/week-*/README.md",
  { query: "?raw", import: "default" },
);

const PATH_RE = /\/docs\/first-14-days\/week-\d+\/day-(\d+)\.md$/;
...
export async function loadWeekReadme(weekNumber: number): Promise<string | undefined> {
  ...
  const key = `/docs/first-14-days/week-${weekNumber}/README.md`;
...
export async function loadDay(dayNumber: number): Promise<Day | undefined> {
  ...
  const day: Day = {
    ...
    dayInWeek: ((frontmatter.day - 1) % 7) + 1,
