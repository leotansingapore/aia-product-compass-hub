 ...
 * SalesPlaybooks
 *
 * Hub landing for everything under "Sales Playbooks" — Scripts, Servicing,
 * Objections, Playbooks, Flows, Concept Cards, Case Vault. Replaces the old
 * behaviour of bouncing users straight into one sub-tab. Now the nav entry
 * lands on /sales-playbooks and the FC chooses which section to open.
 *
 * Each section is its own full page; clicking a card on this hub navigates
 * to that page (which still shows the underline tab bar at the top so the
 * FC can move sideways without coming back here).
 *
 * Accessible to both pre-RNF (papers_taker) and post-RNF tiers — drilling
 * the scripts/objections/case studies is part of pre-RNF prep, not gated
 * behind licensure.
 */
...
interface PlaybookSection {
  ...
  icon: LucideIcon;
  description: string;
  /** Short one-liner under the title — what it's FOR (the job-to-be-done). */
  useFor: string;
  /** Optional badge above the title (count / status). */
  badge?: string;
  /** Tailwind colour ramp for the accent strip + icon background. */
  accent: {
    iconBg: string;
    iconColor: string;
    accent: string;
...
export default function SalesPlaybooksPage() {
  ...
  return (
    <PageLayout
      ...
      description="Scripts, servicing flows, objection handlers, playbooks, concept-card drawings, and the case vault — every sales asset in one place."
    >
      <BrandedPageHeader
        tone="dark"
        showOnMobile
        title="Sales Playbooks"
        subtitle="Everything you need to run a real appointment — scripts, objection handlers, concept cards, and 31 real-prospect case receipts."
        breadcrumbs={[
          { label: "Home", href: "/" },
      ...
      <div className="mx-auto px-3 md:px-6 py-4 md:py-8 max-w-6xl overflow-x-hidden">
        {/* Intro callout — quick "how to use this hub" */}
        ...
        <div className="mt-8 text-center text-xs text-muted-foreground">
          Every section also has its own tab bar at the top, so you can jump
          between Scripts → Objections → Concept Cards → Case Vault without
          coming back to this hub.
        </div>
      </div>
    </PageLayout>
  );
