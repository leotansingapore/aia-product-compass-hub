...
export default function UltimateCriticalCoverStudy() {
  const navigate = useNavigate();
  const [quizSize, setQuizSize] = useState<QuizSize | null>(null);
  const [categoryFilter, setCategoryFilter] = useState<CategoryFilter>('all');
  const [activeQuestions, setActiveQuestions] = useState<typeof ultimateCriticalCoverStudyBank | null>(null);

  const categoryCounts = useMemo(() => {
    const counts: Record<string, number> = {};
    for (const q of ultimateCriticalCoverStudyBank) {
      counts[q.category] = (counts[q.category] || 0) + 1;
    }
    return counts;
  }, []);

  const weakQuestions = useMemo(() => loadWeakQuestions('ultimate-critical-cover'), []);
  ...
  const startQuiz = (size: QuizSize, category: CategoryFilter) => {
    let pool = category === 'all'
      ? ultimateCriticalCoverStudyBank
      : ultimateCriticalCoverStudyBank.filter(q => q.category === category);
  ...
  if (activeQuestions) {
    return (
      <ProtectedPage pageId="ultimate-critical-cover-study">
        <PageLayout
          title="Ultimate Critical Cover Study Mode | FINternship"
          description="Study Ultimate Critical Cover product knowledge with instant feedback."
        >
          <div className="max-w-5xl mx-auto px-3 sm:px-6 py-4 sm:py-8">
            ...
            <StudyQuiz questions={activeQuestions} onFinish={handleRestart} productSlug="ultimate-critical-cover" />
            </div>
            <div className="w-full lg:w-64 shrink-0">
              <div className="lg:sticky lg:top-20">
                <StudyResourcesSidebar productSlug="ultimate-critical-cover" />
              </div>
            </div>
            </div>
          </div>
  ...
  return (
    <ProtectedPage pageId="ultimate-critical-cover-study">
      <PageLayout
        title="Ultimate Critical Cover Study Bank | FINternship"
        description="Study Ultimate Critical Cover product knowledge with practice questions."
      >
        <div className="max-w-5xl mx-auto px-3 sm:px-6 py-4 sm:py-8 animate-fade-in">
          ...
          <Button
            variant="ghost"
            size="sm"
            onClick={() => navigate('/product/ultimate-critical-cover/aia-materials')}
            className="mb-3 -ml-2"
          >
            <ArrowLeft className="h-4 w-4 mr-1" />
            Back to Ultimate Critical Cover
          </Button>

          <h1 className="text-2xl sm:text-3xl font-bold mb-2">
            Ultimate Critical Cover — Study Bank
          </h1>
          <p className="text-muted-foreground mb-6">
            {ultimateCriticalCoverStudyBank.length} practice questions to prepare you for the actual exam.
            ...
            <CardContent>
              <div className="flex flex-wrap gap-2">
                {(['all', 'product-facts', 'sales-angles', 'objection-handling', 'roleplay'] as const).map((cat) => {
                  const Icon = categoryIcons[cat];
                  const count = cat === 'all' ? ultimateCriticalCoverStudyBank.length : (categoryCounts[cat] || 0);
                  return (
                    <button
                      key={cat}
              ...
              <div className="grid grid-cols-3 gap-3">
                {([25, 50, 100] as QuizSize[]).map((size) => {
                  const available = categoryFilter === 'all'
                    ? ultimateCriticalCoverStudyBank.length
          ...
          <div className="mt-6 rounded-lg border border-primary/20 bg-primary/5 p-4">
            ...
            <Button
              size="sm"
              onClick={() => navigate('/product/ultimate-critical-cover/exam')}
            >
