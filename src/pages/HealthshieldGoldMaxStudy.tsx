...
export default function HealthshieldGoldMaxStudy() {
  const navigate = useNavigate();
  const [quizSize, setQuizSize] = useState<QuizSize | null>(null);
  const [categoryFilter, setCategoryFilter] = useState<CategoryFilter>('all');
  const [activeQuestions, setActiveQuestions] = useState<typeof healthshieldGoldMaxStudyBank | null>(null);

  const categoryCounts = useMemo(() => {
    const counts: Record<string, number> = {};
    for (const q of healthshieldGoldMaxStudyBank) {
      counts[q.category] = (counts[q.category] || 0) + 1;
    }
    return counts;
  }, []);

  const weakQuestions = useMemo(() => loadWeakQuestions('healthshield-gold-max'), []);
  ...
  const startQuiz = (size: QuizSize, category: CategoryFilter) => {
    let pool = category === 'all'
      ? healthshieldGoldMaxStudyBank
      : healthshieldGoldMaxStudyBank.filter(q => q.category === category);
  ...
  if (activeQuestions) {
    return (
      <ProtectedPage pageId="healthshield-gold-max-study">
        <PageLayout
  ...
  return (
    <ProtectedPage pageId="healthshield-gold-max-study">
      <PageLayout
        title="HealthShield Gold Max Study Bank | FINternship"
        description="Study HealthShield Gold Max product knowledge with practice questions."
      >
        <div className="max-w-5xl mx-auto px-3 sm:px-6 py-4 sm:py-8 animate-fade-in">
          ...
          <Button
            variant="ghost"
            size="sm"
            onClick={() => navigate('/product/healthshield-gold-max')}
            className="mb-3 -ml-2"
          >
            <ArrowLeft className="h-4 w-4 mr-1" />
            Back to HealthShield Gold Max
          </Button>

          <h1 className="text-2xl sm:text-3xl font-bold mb-2">
            HealthShield Gold Max — Study Bank
          </h1>
          <p className="text-muted-foreground mb-6">
            {healthshieldGoldMaxStudyBank.length} practice questions to master hospital plan product knowledge.
            ...
            <CardContent>
              <div className="flex flex-wrap gap-2">
                {(['all', 'product-facts', 'sales-angles', 'objection-handling', 'roleplay'] as const).map((cat) => {
                  const Icon = categoryIcons[cat];
                  const count = cat === 'all' ? healthshieldGoldMaxStudyBank.length : (categoryCounts[cat] || 0);
                  return (
                    <button
                      key={cat}
              ...
              <div className="grid grid-cols-3 gap-3">
                {([25, 50, 100] as QuizSize[]).map((size) => {
                  const available = categoryFilter === 'all'
                    ? healthshieldGoldMaxStudyBank.length
