...
serve(async (req) => {
  ...
  try {
    // This function holds the service-role key and can delete/rewrite the whole
    // scripts corpus, so every mode requires a signed-in caller; the destructive
    // and seeding modes require an admin.
    const caller = await identifyCaller(req);
    if (!caller.userId) return denied(corsHeaders, "Sign in to modify scripts", 401);

    const body = await req.json();
    const { scripts, mode, targetId, newVersion } = body;

    const adminOnlyMode = mode === "bulk-update" || mode === "add" || !mode;
    if (adminOnlyMode && !caller.isAdmin) {
      return denied(corsHeaders, "Only admins can seed or bulk-edit scripts");
    }
