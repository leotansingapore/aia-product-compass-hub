...
BEGIN
  ...
  visible_users AS (
    SELECT tu.user_id
    FROM tier_users tu
    LEFT JOIN public.profiles pr ON pr.user_id = tu.user_id::text
  ...
  assembled AS (
    SELECT
      vu.user_id,
      pr.display_name, pr.first_name, pr.last_name, pr.email,
    ...
    LEFT JOIN public.profiles pr ON pr.user_id = vu.user_id::text
  ...
  scored AS (
    SELECT
      ...
      (a.f14_quiz * 5)
        ...
        + (a.asg_count * 50)
        + (a.qb_count * 1)
        + (a.vid_count * 2.5) AS total_points,
      a.days_active,
      a.f14_quiz * 5     AS first_14_days,
      a.f60_quiz * 5     AS first_60_days,
      a.asg_count * 50   AS assignments,
      a.qb_count * 1     AS question_bank,
      a.vid_count * 2.5  AS videos
    FROM assembled a
  )
  SELECT
    s.user_id, s.name, s.email, s.total_points, s.days_active,
