-- Phase 1: Create new tables for cleaner role/tier separation
...
-- Migrate existing data from user_roles to new structure
INSERT INTO public.user_access_tiers (user_id, tier_level, granted_at)
SELECT 
  user_id,
  CASE 
    WHEN role IN ('basic', 'intermediate', 'advanced') THEN role
    ELSE 'basic'
  END as tier_level,
  created_at
FROM public.user_roles 
WHERE role IN ('basic', 'intermediate', 'advanced', 'user')
