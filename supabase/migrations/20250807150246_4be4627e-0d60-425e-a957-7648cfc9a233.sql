-- Clean up conflicting functions and ensure proper UUID handling
DROP FUNCTION IF EXISTS public.get_user_tier(text);
DROP FUNCTION IF EXISTS public.has_role(text, text);

-- Check what data type user_id actually is in user_roles table
-- If it's text, we need to convert UUID to text in the function
...
-- Update has_tier_access function to use UUID properly
