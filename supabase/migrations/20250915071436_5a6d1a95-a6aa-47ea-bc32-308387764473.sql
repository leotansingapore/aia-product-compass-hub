-- Phase 1: Create new role/tier system with proper constraint handling

-- First, update the check constraint to allow 'active' status
ALTER TABLE public.user_approval_requests 
DROP CONSTRAINT user_approval_requests_status_check;

ALTER TABLE public.user_approval_requests 
ADD CONSTRAINT user_approval_requests_status_check 
CHECK (status = ANY (ARRAY['pending'::text, 'active'::text, 'suspended'::text, 'approved'::text, 'rejected'::text]));
...
-- Now update status values after constraint is fixed
