-- 1. product_mastery_progress table
...
CREATE POLICY "Users can view own product mastery progress"
  ON public.product_mastery_progress FOR SELECT
  USING (auth.uid() = user_id);

CREATE POLICY "Users can insert own product mastery progress"
  ON public.product_mastery_progress FOR INSERT
  WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update own product mastery progress"
  ON public.product_mastery_progress FOR UPDATE
  USING (auth.uid() = user_id);

CREATE POLICY "Users can delete own product mastery progress"
  ON public.product_mastery_progress FOR DELETE
  USING (auth.uid() = user_id);

CREATE INDEX idx_product_mastery_progress_passed_user
  ON public.product_mastery_progress (user_id)
  WHERE quiz_passed_at IS NOT NULL;

-- 2. Leaderboard RPC: drop + recreate (return type changed)
...
CREATE OR REPLACE FUNCTION public.get_learner_leaderboard(p_tier text)
 RETURNS TABLE(user_id uuid, name text, email text, total_points numeric, days_active integer, first_14_days numeric, first_60_days numeric, next_60_days numeric, assignments numeric, question_bank numeric, videos numeric, product_mastery numeric)
 LANGUAGE plpgsql
...
BEGIN
  ...
  days AS (
    ...
    FROM (
      ...
      UNION SELECT user_id, DATE(quiz_passed_at) FROM public.product_mastery_progress WHERE quiz_passed_at IS NOT NULL
      UNION SELECT user_id, DATE(updated_at) FROM public.product_mastery_progress WHERE updated_at IS NOT NULL
      UNION SELECT user_id, DATE(COALESCE(submitted_at, created_at)) FROM public.assignment_submissions
        WHERE product_id IN ('first-60-days-assignments', 'next-60-days-assignments') AND COALESCE(submitted_at, created_at) IS NOT NULL
  ...
  assembled AS (
    SELECT
      ...
      COALESCE(pm.pm_quiz, 0)   AS pm_quiz,
  ...
  scored AS (
    SELECT
      ...
      COALESCE(NULLIF(TRIM(a.display_name), ''),
               ...
               'Anonymous') AS name,
      a.email,
      (a.f14_quiz * 5) + (a.f60_quiz * 5) + (a.n60_quiz * 5) + (a.pm_quiz * 5) + (a.asg_count * 50)
      ...
      a.pm_quiz * 5 AS product_mastery
    FROM assembled a
  ...
  SELECT s.user_id, s.name, s.email, s.total_points, s.days_active,
    s.first_14_days, s.first_60_days, s.next_60_days, s.assignments, s.question_bank, s.videos, s.product_mastery
  FROM scored s
