...
#### Seed the 5 Project 200 warm-market openers into the `scripts` table — DONE 2026-06-10

Migration `20260609160654_cbe82d5e-9324-4165-ae2b-4afee894ff5e.sql` (commit `9ec4294c`). Lovable inserted the 5 canonical Project 200 openers into `public.scripts` with `target_audience = 'warm-market'`, `category = 'prospecting'`, sort_order 950-970, message in `versions[].content`. Verified against the live DB: all 5 "Warm Market Opener -" rows present, verbatim, no duplicates, surfacing under `/scripts?audience=warm-market`. The Project 200 assignment ([`assignment-02.md`](docs/first-60-days/assignments/assignment-02.md)) already links to that filter, so the link now resolves to them. Optional follow-up still open: swap the inline openers in the assignment for the link only (kept inline for now as a working aid).
...
Migration `20260424062912_df04d873-*.sql` shipped: `CREATE OR REPLACE FUNCTION public.get_learner_leaderboard(p_tier text)` swapped two `WHERE product_id = 'first-60-days-assignments'` clauses for `WHERE product_id IN ('first-60-days-assignments', 'next-60-days-assignments')` — one inside the `asg` CTE (line 50), one inside the `days` UNION (line 74). RETURNS TABLE shape unchanged. `SECURITY DEFINER`, `STABLE`, `SET search_path = public`, `#variable_conflict use_column`, and `GRANT EXECUTE ... TO authenticated` all preserved. Verified: function body now contains both filter values; no existing `next-60-days-assignments` submissions yet so no points moved on existing learners — but future Next 60 Days assignment submissions will now credit at × 50, aggregated into the same `assignments` breakdown column.
